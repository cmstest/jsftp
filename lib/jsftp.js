...
Ftp.prototype._createSocket = function(port, host, firstAction) {
  ...
  if (this.ssl) {
    this.socket = tls.connect(port, host, this.sslOptions, function() {
  ...
  self.raw.user(user, function(err, res) {
    ...
    self.raw.pass(pass, function(err, res) {
      ...
      if (err) {
        ...
        // Check for PRET Support
        self.getFeatures(function(err, feats) {
          if(self.hasFeat("pret") === true) {
            this.usePRET = true;
          } else {
            this.usePRET = false;
          }
...
Ftp.prototype.getPasvSocket = function(callback) {
  ...
  this.execute('pasv', function(err, res) {
    ...
    getPasvPort(res.text, function(err, options) {
      ...
      var socket;
      if (self.ssl) {
        socket = tls.connect(res.port, res.host, options, function() {
          self.reemit('connect');
        });
      }
      else{
